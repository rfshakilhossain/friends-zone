...
class UserService {
  ...
  Future<void> ensureProfile(User user, {String? displayName}) async {
    ...
    if (existing == null) {
      await _client.from('profiles').insert({
        'id': user.id,
        'display_name': (displayName ?? user.userMetadata?['display_name'] ?? user.email?.split('@').first ?? 'Friends Zone User').toString().trim(),
        'email': user.email,
  ...
  Stream<Map<String, dynamic>?> currentUserStream() {
    ...
    return _client.from('profiles').stream(primaryKey: ['id']).eq('id', user.id).map((rows) => rows.isEmpty ? null : rows.first);
  }

  Stream<List<Map<String, dynamic>>> visibleUsersStream() {
    return _client.from('profiles').stream(primaryKey: ['id']).eq('visibility', true).order('is_online', ascending: false);
  }

  Future<void> updateProfile({String? displayName, String? bio, String? avatarUrl, bool? visibility}) async {
    final user = _client.auth.currentUser;
    if (user == null) return;
